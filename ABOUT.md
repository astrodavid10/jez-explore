...
**Jezero Explorer** is a zoomable map of Jezero Crater on Mars: the 45-kilometre impact basin
that held a lake and a river delta more than three billion years ago, where NASA's
*Perseverance* rover landed on 18 February 2021 and where *Ingenuity* made the first 72
powered flights on another world.
...
companion to the planetarium's Mars programmes.

*Data to Dome, Dome to Phone:* the same NASA data that drives the 20-metre dome downtown
...
decodes real metres out of the pixel colours — that is the single source for the contour lines,
...
Vertical values are untouched — true Mars metres on the areoid. The app divides that factor
...
surface of constant gravity that stands in for one. Jezero's floor sits about 2,600 metres
*below* that zero, so contour labels read "−2,450 m" and so on. The rover is now more than 600
metres higher than where it landed, and still below zero.
